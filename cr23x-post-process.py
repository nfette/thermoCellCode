...
data = np.genfromtxt(myfile, delimiter=",",names=True, usecols=range(1,cols),
                     skip_footer=2)
...
dataT = np.genfromtxt(myfile, delimiter=",",names=True,
                      usecols=[0], dtype='datetime64', converters={0:fun1},
                      skip_footer=2)
a = np.nonzero(np.diff(dataT) < datetime.timedelta(minutes=-1))[0]
for i in a:
    dataT[1+i:] += datetime.timedelta(1)
